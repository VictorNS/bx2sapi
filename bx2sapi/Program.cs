...
Example:
  d:\bx2sapi\test.csv /minpause:4000 /silentpause:4500");
				...
				using (var fs = new FileStream(outFileRuEn, FileMode.OpenOrCreate))
				{
					using (var writer = new StreamWriter(fs, System.Text.Encoding.GetEncoding(1251)))
					{
						foreach (var raw in data)
						{
							...
							if (-1 == raw.RusTran.IndexOf('('))
								...
								writer.WriteLine(@"listen<silence msec=""400""/><lang langid=""419"">{0}</lang><silence msec=""400""/>translate{2}<lang langid=""419"">{1} {3}</lang>"
						...
						writer.WriteLine(@"The lesson is over.<silence msec=""400""/>Come back and listen to me again!<silence msec=""5000""/>");
					}
				}
				Console.WriteLine(outFileRuEn);
				#endregion read RuEn file
				#region write EnRu file
				using (var fs = new FileStream(outFileEnRu, FileMode.OpenOrCreate))
				{
					using (var writer = new StreamWriter(fs, System.Text.Encoding.GetEncoding(1251)))
					{
						foreach (var raw in data)
						{
							...
							if (-1 == raw.RusTran.IndexOf('('))
								...
								writer.WriteLine(@"<silence msec=""{2}""/><lang langid=""419"">{0}</lang><silence msec=""800""/><lang langid=""419"">{1}</lang>"
